  ...
  return (
    <div className="glass-panel p-6 md:p-8 rounded-2xl space-y-8 shadow-2xl ring-1 ring-white/10 animate-fade-in-up">
       ...
       {settings.contentType === ContentType.SPECIFIC ? (
          <div>
            <label htmlFor="searchInput" className="block text-sm font-semibold mb-3 text-gray-300 uppercase tracking-wider">{isArabic ? 'اسم الأنمي' : 'Name'}</label>
       ...
       {isRoomSetup && (
         <div className="pt-4 border-t border-white/10 space-y-4">
            <div>
               <label htmlFor="playerName" className="block text-sm font-semibold mb-2 text-green-400">Host Name</label>
               <input id="playerName" type="text" aria-label="Player Name" value={playerName} onChange={e => onPlayerNameChange(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white" />
            </div>
            <div>
               <label htmlFor="hostApiKey" className="block text-sm font-semibold mb-2 text-green-400">Gemini API Key (Required for Host)</label>
               <input id="hostApiKey" type="password" aria-label="Host API Key" value={hostApiKey} onChange={e => onHostApiKeyChange(e.target.value)} placeholder="AIza..." className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white" />
       ...
       {!isRoomSetup && (
         <div className="pt-4 border-t border-white/10 space-y-4">
            <div>
               <label htmlFor="hostApiKeySingle" className="block text-sm font-semibold mb-2 text-gray-400">{isArabic ? 'مفتاح API (اختياري)' : 'Gemini API Key (Optional)'}</label>
               <input
                 id="hostApiKeySingle"
                 type="password"
